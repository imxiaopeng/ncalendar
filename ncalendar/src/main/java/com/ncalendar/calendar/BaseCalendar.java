...
import com.ncalendar.enumeration.CalendarBuild;
import com.ncalendar.enumeration.CheckModel;
import com.ncalendar.enumeration.DateChangeBehavior;
import com.ncalendar.enumeration.MultipleCountModel;
import com.ncalendar.listener.OnCalendarChangedListener;
import com.ncalendar.listener.OnCalendarMultipleChangedListener;
import com.ncalendar.listener.OnClickDisableDateListener;
import com.ncalendar.listener.OnMWDateChangeListener;
import com.ncalendar.painter.CalendarAdapter;
import com.ncalendar.painter.CalendarBackground;
import com.ncalendar.painter.CalendarPainter;
import com.ncalendar.painter.InnerPainter;
import com.ncalendar.painter.NumBackground;
import com.ncalendar.painter.WhiteBackground;
import com.ncalendar.utils.Attrs;
import com.ncalendar.utils.AttrsUtil;
import com.ncalendar.view.ICalendarView;
...
public abstract class BaseCalendar extends ViewPager implements ICalendar {
    ...
    private LocalDate pagerInitialDate;

    public BaseCalendar(@NonNull Context context, AttributeSet attributeSet) {
        super(context.getApplicationContext(), attributeSet);
        this.mAttrs = AttrsUtil.getAttrs(context.getApplicationContext(), attributeSet);
    ...
    private void drawView(int position) {
        ...
        if (iCalendarView == null) {
            //单选
            pagerInitialDate = pagerInitialDate == null ? mStartDate : pagerInitialDate;
            if (mOnCalendarChangedListener != null && mCheckModel != CheckModel.MULTIPLE && getVisibility() == VISIBLE) {
                mOnCalendarChangedListener.onCalendarChange(BaseCalendar.this, pagerInitialDate.getYear(), pagerInitialDate.getMonthOfYear(), pagerInitialDate, mDateChangeBehavior);
            }
            return;
        }
    ...
    protected void jump(LocalDate localDate, boolean isCheck, DateChangeBehavior dateChangeBehavior) {
        ...
        if (!isAvailable(localDate)) {
            ...
            pagerInitialDate = localDate;
            ICalendarView iCalendarView = findViewWithTag(getCurrentItem());
            if (iCalendarView != null) {
                pagerInitialDate = iCalendarView.getPagerInitialDate();
            }
            //得出两个页面相差几个
            int indexOffset = getTwoDateCount(localDate, pagerInitialDate, mFirstDayOfWeek);
            if (isCheck) {
